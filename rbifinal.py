...
class WalletApp:
    def __init__(self, root):
        ...
        self.balance_value = tk.Label(root, text=f"₹ {self.current_user.get_balance()}")
        ...
        # Buttons
        self.deposit_button = tk.Button(root, text="Deposit", command=self.deposit , bg="lightblue",fg="white")
        ...
        self.send_button = tk.Button(root, text="Send Money", command=self.send_money , bg="green" , fg="yellow")
    ...
    def deposit(self):
        try:
            ...
            self.update_balance()
            messagebox.showinfo("Deposit", f"Deposited ₹ {amount}. Transaction ID: {transaction_id}")
    ...
    def withdraw(self):
        try:
            ...
            self.update_balance()
            messagebox.showinfo("Withdrawal", f"Withdrew ₹ {amount}. Transaction ID: {transaction_id}")
        except ValueError as e:
            messagebox.showerror("Error", str(e))
    ...
    def send_money(self):
        try:
            ...
            self.update_balance()
            messagebox.showinfo("Send Money", f"Sent ₹ {amount} to {recipient_name}")
        except ValueError as e:
            messagebox.showerror("Error", str(e))
        ...
        if transactions:
            ...
            for transaction in transactions:
                if transaction[2] == 'Sent':
                    history_str += f"Transaction ID: {transaction[0]}, Type: {transaction[2]}, Amount: ₹ {transaction[3]}, To: {transaction[4]}\n"
                else:
                    serial_number_str = transaction[1] if transaction[2] != 'Received' or self.current_user.user_name != transaction[4] else 'Hidden'
                    history_str += f"Transaction ID: {transaction[0]}, Serial Number: {serial_number_str}, Type: {transaction[2]}, Amount: ₹ {transaction[3]}\n"
    ...
    def show_serial_numbers(self):
        ...
        if transactions:
            ...
            for transaction in transactions:
                serial_number_str = transaction[1] if transaction[2] != 'Received' or self.current_user.user_name != transaction[4] else 'Hidden'
                serial_numbers_str += f"Serial Number: {serial_number_str}, Amount: ₹ {transaction[3]}\n"
    ...
    def update_balance(self):
        self.balance_value.config(text=f"₹ {self.current_user.get_balance()}")
